...
```text
11_object_storage_stream_outpu.png
```
...
Цель задания — построить дашборды и графики в Yandex DataLens для визуализации данных, полученных в результате выполнения предыдущих ETL-заданий.

Для визуализации использовались данные, полученные после обработки JSON-сообщений из Kafka в задании 3.

### Источник данных

В задании 3 PySpark-задания прочитали JSON-сообщения из Kafka topic:

```text
loan-events-topic
```

Затем вложенный JSON был преобразован в плоскую структуру и сохранён в Object Storage в CSV-формате.

Для DataLens использовался preview-файл из Object Storage:

```text
task3/output/loan_events_flatten_preview/
```

Файл содержит плоскую таблицу со следующими полями:
...
В Yandex DataLens было создано файловое подключение к CSV-файлу:

```text
loan-events-flatten-file-connection
```

После загрузки файла DataLens корректно распознал структуру данных и поля таблицы.

На основе подключения был создан датасет:

```text
loan_events_flatten_dataset
```

Далее были построены визуализации для анализа кредитных заявок.
...
```text
1. Количество заявок по регионам
2. Распределение заявок по уровню риска
3. Количество заявок по статусам решений
4. Средняя сумма кредита по регионам
5. Средний скоринг по уровню риска
```
...
График показывает распределение количества кредитных заявок по регионам.

Используемые поля:

```text
X: region
Y: COUNT(application_id)
```

Этот график позволяет определить, из каких регионов поступило больше всего заявок.

#### Распределение заявок по уровню риска

Круговая диаграмма показывает распределение заявок по уровню риска:
...
```text
Категории: risk_level
Показатель: COUNT(application_id)
```

График позволяет оценить структуру заявок с точки зрения скорингового риска.

#### Количество заявок по статусам решений

График показывает количество заявок по статусам решений:
...
Этот график помогает понять, сколько заявок было одобрено, отклонено, отправлено на ручную проверку или осталось в ожидании.

#### Средняя сумма кредита по регионам

График показывает среднюю сумму кредита в разрезе регионов.
...
Он позволяет сравнить средний размер запрашиваемого кредита между регионами.

#### Средний скоринг по уровню риска

График показывает среднее значение скорингового балла для каждого уровня риска.
...
Этот график используется для проверки логики распределения заявок по risk level.

### Скриншоты по заданию 4

Скриншоты находятся в папке:

```text
task4_datalens/screenshots/
```

Список скриншотов:

```text
01_datalens_file_connection_preview.png
```

Создано файловое подключение к CSV-файлу. DataLens корректно отображает preview данных и распознаёт колонки плоской таблицы.

```text
02_chart_applications_by_region.png
```

Построен график количества заявок по регионам.

```text
03_chart_risk_level_distribution.png
```

Построена круговая диаграмма распределения заявок по уровню риска.

```text
04_chart_decision_status_count.png
```

Построен график количества заявок по статусам решений.

```text
05_chart_avg_amount_by_region.png
```

Построен график средней суммы кредита по регионам.

```text
06_chart_avg_score_by_risk_level.png
```

Построен график среднего скорингового балла по уровню риска.

### Результат

В результате выполнения задания был создан набор визуализаций в Yandex DataLens на основе данных, полученных после потоковой обработки JSON-сообщений из Kafka.

Данные были предварительно обработаны PySpark-заданиями в Yandex Data Processing: вложенный JSON был разложен в плоскую таблицу, сохранён в Object Storage и затем использован как источник данных для DataLens.

Построенные графики позволяют анализировать кредитные заявки по регионам, статусам решений, уровню риска, суммам кредита и скоринговым баллам.
